...
type ExtendedProps = NonNullable<Awaited<ReturnType<typeof getMedicalRecords>>['data']>[number];
...
const MedicalRecordsPage = async (props: SearchParamsProps) => {
    ...
    const renderRow = (item: ExtendedProps) => {
        const name = item.patient ? `${item.patient.firstName} ${item.patient.lastName}` : 'N/A';
            ...
            >
                ...
                <td className='hidden lg:table-cell'>
                    {!item?.encounter || item?.encounter?.length === 0 ? (
                        ...
                        <span>{item?.encounter.length}</span>
                    )}
                </td>
