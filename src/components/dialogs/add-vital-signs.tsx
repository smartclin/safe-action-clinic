...
import { VitalSignsSchema } from '@/schema';
...
export const AddVitalSigns = ({ patientId, doctorId, appointmentId, medicalId }: AddVitalSignsProps) => {
    ...
    const form = useForm<VitalSignsFormData>({
        resolver: zodResolver(VitalSignsSchema) as unknown as Resolver<VitalSignsFormData>,
        defaultValues: {
            patientId: patientId,
            medicalId: medicalId,
    ...
    const handleOnSubmit = async (data: VitalSignsFormData) => {
        try {
            ...
            if (res?.data?.success) {
                ...
                toast.error(res?.data?.message || 'Failed to add vital signs');
            }
        } catch (error) {
            console.log(error);
